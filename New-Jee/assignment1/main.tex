...
\item[18.] Let the line $y = mx$ and the ellipse $2x^2 + y^2 = 1$ intersect a point $P$ in the first quadrant. If the 
	...
	, then $\beta$ is equal to 
		...
		\begin{tikzpicture}
    			\draw[blue, thick, domain=0:360, samples=100] 
    			...
    			\draw[orange, thick, domain=-0.7:0.5, samples=100]
        			plot ({\x}, {2*(\x) + 0.4714});
    
    			\fill[red] (0, 0.4714) circle (1pt) node[below] {\tiny $(0, \beta)$};
    			\fill[red] ({-1/(3*sqrt(2))}, 0) circle (1pt) node[below left] {\tiny $(\frac{-1}{3\sqrt{2}}, 0)$};
    			\fill[red] (0.235, 0.942) circle (1pt) node[above right] {\tiny $P$};		
		\end{tikzpicture}
...
\\
	\item[22.] Let the normal at a point $P$ on the curve $y^2 - 3x^2 + y + 10 = 0$ intersect the y-axis at 
		...
		\centering
		\begin{tikzpicture}
   			\draw[green, thick] (-5, 0) -- (5, 0) node[right] {$x$};
			\draw[green, thick] (0, -5) -- (0, 5) node[above] {$y$};
    
    			\draw[very thin, gray] (-6,-6) grid (6,6);
    
			\draw[domain=-3.5:-1.8027757,smooth,variable=\x,blue] plot ({\x},{(-1 + sqrt(1 + 12*(\x)^2 - 40))/2});
			\draw[domain=-3.5:-1.8027757,smooth,variable=\x,blue] plot ({\x},{(-1 - sqrt(1 + 12*(\x)^2 - 40))/2});
    
			\draw[domain=1.8027757:3.5,smooth,variable=\x,blue] plot ({\x},{(-1 + sqrt(1 + 12*(\x)^2 - 40))/2});
			\draw[domain=1.8027757:3.5,smooth,variable=\x,blue] plot ({\x},{(-1 - sqrt(1 + 12*(\x)^2 - 40))/2});
    			\draw[red, thick, domain=0.5:3, samples=100]
				plot ({\x}, {4 * (\x) - 7});
    			\draw[red, thick, domain=-3:6, samples=100]
				plot ({\x}, {-0.25 * (\x) + 1.5});
    			\fill[red] (2, 1) circle (3pt) node[left] {\small $(2, 1)$};
			\fill[red] (0, 1.5) circle (3pt) node[above] {\small $(0, \frac{3}{2})$};
		\end{tikzpicture}
		\end{figure}
\\	
